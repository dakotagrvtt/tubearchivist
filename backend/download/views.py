...
class DownloadApiView(ApiBaseView):
    ...
    def post(self, request, video_id):
        ...
        if item_status == "ignore-force":
            # The video page deletes the media and index document before this
            # request. Recreate a minimal ignore marker directly in the queue.
            ignore_doc = {
                "youtube_id": video_id,
                "status": "ignore",
        ...
        if status_code not in (200, 201):
            error = ErrorResponseSerializer(
                {"error": "download queue lookup failed"}
            )
